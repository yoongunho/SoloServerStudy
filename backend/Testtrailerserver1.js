...
import fs from 'fs';
import { PORT, HTTP_PORT, SSL_CERT, SSL_KEY, CORS_HEADERS, ADMIN_PATH } from './config.js';
import { handleRegister }                      from './routes/register.js';
import { handleStatic }                        from './routes/static.js';
import { handleTrailerGet, handleAdminUpdate } from './routes/admin.js';
...
https.createServer(sslOptions, handleRequest).listen(PORT, '0.0.0.0', () => {
  console.log(`HTTPS 서버 실행 중: https://studyserver.mooo.com`);
});

// ── HTTP → HTTPS 리다이렉트 (80포트) ────────────────────
http.createServer((req, res) => {
  res.writeHead(301, { Location: `https://studyserver.mooo.com${req.url}` });
  res.end();
